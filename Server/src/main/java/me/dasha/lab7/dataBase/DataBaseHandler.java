...
public class DataBaseHandler {
    ...
    public static void connectToDb() {
        ...
        try (InputStream in = Files.newInputStream(Paths.get("database.properties"))){
            ...
            logger.severe("Ошибка при попытке подсоединения");
            e.printStackTrace();
            System.exit(1);
    ...
    public static Stack<SpaceMarine> loadCollection() {
        try {
            ...
            logger.info("Коллекция загружена. Количество элементов - " + stack.size());
            return stack;
        } catch (SQLException e) {
